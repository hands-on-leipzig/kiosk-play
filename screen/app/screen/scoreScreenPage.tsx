...
export default function ScoreScreenPage() {
    ...
    function renderScoreCell(score: Score, index: number) {
        ...
        return (
            <td className="px-4 border-t border-r border-l border-black text-center" key={index} style={{ background }}>
                {score.points}
            </td>
        );
    }
        ...
        <ScreenContainer settings={settings}>
            ...
            <div className="text-gray text-5xl rounded-lg p-20">
                ...
                <table className="w-full border-collapse table-fixed text-left text-gray ">
                    <thead>
                        <tr>
                            <th className="px-4 py-2 border-b border-r border-black w-auto">Team</th>
                            {round === 'VR' ? (
                                <>
                                    <th className="px-4 py-2 border-r border-b border-black text-center w-40">R I</th>
                                    <th className="px-4 py-2 border-r border-b border-black text-center w-40">R II</th>
                                    <th className="px-4 py-2 border-r border-b border-black text-center w-40">R III</th>
                                </>
                            ) : (
                                <th className="px-4 py-2 border-r border-b border-black text-center w-60">Score</th>
                            )}
                            <th className="px-4 py-2 border-b border-black text-center w-40">Rank</th>
                        </tr>
                    </thead>
                    <tbody>
                        {teams?.slice(currentIndex, Math.min(currentIndex + teamsPerPage, teams?.length)).map((team: Team) => (
                            <tr key={team.id}>
                                <td className="px-4 py-2 border-t border-black">{team.name}</td>
                                {team.scores.map((score, index) => renderScoreCell(score, index))}
                                <td className="px-4 py-2 border-t border-black text-center">{team.rank}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
